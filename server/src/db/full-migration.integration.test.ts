...
// Run: RUN_INTEGRATION=1 npm run test --workspace=server -- src/db/full-migration.integration.test.ts
...
const chatSchemaSql = readFileSync(
	new URL("./chat-schema.sql", import.meta.url),
	"utf8",
);
const runIntegration = Boolean(process.env.RUN_INTEGRATION);
const schemaName = `full_migration_${process.pid}_${randomUUID().replaceAll("-", "")}`;
...
async function applyCanonicalMigration(client: Client) {
	await client.query("BEGIN");
	try {
		await client.query(schemaSql);
		await client.query(agentSchemaSql);
		await client.query(chatSchemaSql);
...
async function assertMigrationArtifacts(client: Client) {
	const statusNullability = await row<{ is_nullable: string }>(
		client,
		`SELECT is_nullable FROM information_schema.columns
		 WHERE table_schema = current_schema()
		   AND table_name = 'cards' AND column_name = 'status_id'`,
		[],
	);
	expect(statusNullability?.is_nullable).toBe("NO");
	...
	expect(tables.rows.map((entry) => entry.table_name).sort()).toEqual(
		[
			"agent_boards",
			"chat_messages",
			"chat_threads",
	...
	const statusSlotIndex = await row<{ indexname: string }>(
		client,
		`SELECT indexname FROM pg_indexes
		 WHERE schemaname = current_schema()
		   AND tablename = 'tracker_vocabularies'
		   AND indexdef ILIKE '%workspace_id%slot%'`,
		[],
	);
	expect(statusSlotIndex?.indexname).toBeTruthy();
}
...
async function setupFreshLegacy(client: Client): Promise<Fixture> {
	await applySchemaBeforeSlotSeed(client);
	await client.query(
		`INSERT INTO users (username, display_name, password_hash)
		 VALUES ('full-migration-owner', 'Full Migration Owner', 'test')`,
	);
	const workspaceId = await workspace(client, "full-migration-fresh", 0);
	...
	await applyCanonicalMigration(client);
	await assertMigrationArtifacts(client);
	await assertFinalStatusNotNull(client, workspaceId);
	return { workspaceId, before, expected: freshExpected };
}

async function setupExistingLegacy(client: Client): Promise<Fixture> {
	await applySchemaBeforeSlotSeed(client);
	await client.query(
		`INSERT INTO users (username, display_name, password_hash)
		 VALUES ('full-migration-existing-owner', 'Full Migration Existing Owner', 'test')`,
	);
	const workspaceId = await workspace(client, "full-migration-existing", 4);
	await client.query(agentSchemaSql);
...
describe.skipIf(!runIntegration)("canonical full migration", () => {
	let fresh: Fixture;

	beforeAll(async () => {
		await pool.query(`CREATE SCHEMA ${quotedSchema}`);
		await withSchema(async (client) => {
			fresh = await setupFreshLegacy(client);
		});
	});
	...
	it("applies schema.sql, agent-schema.sql, and chat-schema.sql in one transaction on a fresh schema", async () => {
		await withSchema(async (client) => {
	...
	it("migrates pre-unify human and agent boards, tracker keys, and partial identity", async () => {
		await pool.query(`CREATE SCHEMA ${existingQuotedSchema}`);
		await withSchema(async (client) => {
			const fixture = await setupExistingLegacy(client);
