...
async function seed() {
  ...
  const cards = [
    { col: 0, title: "Set up CI pipeline", desc: "GitHub Actions: lint, test, build." },
    { col: 0, title: "Dark mode support", desc: "Respect prefers-color-scheme." },
    { col: 1, title: "Connect GitHub issues", desc: "Sync open issues into the board." },
    { col: 1, title: "Card labels", desc: "Color-coded labels per card." },
    { col: 2, title: "Board drag & drop", desc: "Move cards between columns." },
    { col: 3, title: "Project scaffolding", desc: "Vite + React + Tailwind v4 + Express + Postgres." },
  ];
    ...
    const res = await pool.query(
      workspaceId == null
        ...
        : `INSERT INTO cards (column_id, title, description, position, created_at, started_at, done_at, workspace_id)
           ...
           RETURNING id`,
      workspaceId == null
        ? [columnId, card.title, card.desc, (i + 1) * POSITION_GAP, isStarted, isDone]
        : [columnId, card.title, card.desc, (i + 1) * POSITION_GAP, isStarted, isDone, workspaceId],
    );
    await pool.query(
      workspaceId == null
