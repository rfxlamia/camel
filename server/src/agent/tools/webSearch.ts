...
function classifyError(err: unknown): SearchErrorCode {
  ...
  if (/timeout|ETIMEDOUT|ECONNRESET|ENOTFOUND|fetch failed|network/i.test(message)) {
...
function isRetryable(code: SearchErrorCode): boolean {
  return code === "RATE_LIMIT" || code === "NETWORK_ERROR" || code === "UNKNOWN";
}
...
async function searchWithRetry(query: string, apiKey: string): Promise<ToolResult> {
