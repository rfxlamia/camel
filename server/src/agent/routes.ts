  ...
  insertBoard: async (data) => {
    const { rows } = await pool.query(
      `INSERT INTO agent_boards (workspace_id, user_id, template_id, original_intent, status)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id`,
      [data.workspaceId, data.userId, data.templateId, data.originalIntent, data.status],
    );
    return { id: rows[0].id };
  },
  ...
  insertOutput: async (data) => {
    await pool.query(
      `INSERT INTO agent_card_outputs (board_id, column_slug, card_index, output, thinking)
       VALUES ($1, $2, $3, $4, $5)`,
      [data.boardId, data.columnSlug, data.cardIndex, data.output, data.thinking ?? null],
    );
  },
