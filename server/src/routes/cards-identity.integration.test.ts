	...
	() => {
		...
		it("allocates real agent cards on board columns without card activity", async () => {
			...
			for (let attempt = 0; attempt < 80; attempt += 1) {
				...
				if (
					board.rows[0]?.execution_status === "done" ||
					board.rows[0]?.execution_status === "failed"
				)
					break;
			}
		...
		it("rejects a direct null status insert after final migration", async () => {
			...
			// A database that has not run the final migration cannot exercise this
			// characterization yet; the static migration test covers its ordering.
			if (nullability.rows[0]?.is_nullable === "YES") return;
			const columnId = await addColumn("Inbox", 1024);
			await expect(
				pool.query(
