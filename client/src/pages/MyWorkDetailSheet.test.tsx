...
vi.mock("../features/my-work", async (importOriginal) => {
	const actual = await importOriginal<typeof import("../features/my-work")>();
	return {
		...actual,
...
async function openReadyDetail(item: MyWorkItem, initialEntry = "/my-work") {
	...
	fireEvent.click(
		screen.getByRole("button", {
			name: new RegExp(`open ${item.key} ${item.title}`, "i"),
		}),
	);
	const detail = await screen.findByRole("dialog", {
		name: new RegExp(item.key, "i"),
