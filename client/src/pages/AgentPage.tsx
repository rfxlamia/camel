    ...
    case "agent.card.started":
      return (
        <p className="text-sm text-neutral-700">
          <span className="font-medium">Started</span> {event.columnSlug ?? "card"}
        </p>
      );
    ...
    case "agent.card.failed":
      return (
        <p className="text-sm text-error-900">
          ...
          {event.columnSlug ?? "Card"} failed{event.error ? `: ${event.error}` : ""}
        </p>
      );
    default:
      return (
...
export default function AgentPage() {
  ...
  const isStreaming =
    isRunning &&
    agentEvents.some((e) => e.type === "agent.card.token");
