...
function createMutationClickHandler({
	item,
	identity,
	pending,
	state,
...
function buildActionController({
	item,
	state,
	snapshot,
	pending,
	...
	reasonId,
	handleClick,
}: ControllerStateOptions): MyWorkDoneActionController {
...
export function useMyWorkDoneAction({
	...
	const handleClick = createMutationClickHandler({
		...
		state,
		setState,
		execute: mutation ?? onMarkDone ?? markDone ?? onMutate ?? markWorkItemDone,
		onOptimisticRemove,
		onSuccess,
		onRollback,
		onRefresh,
