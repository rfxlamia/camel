...
describe("ToolTrace", () => {
  it("renders collapsed by default with a one-line summary", () => {
    const { container } = render(
      <ToolTrace steps={[{ toolName: "web_search", query: "X", resultCount: 10, columnSlug: "test" }]} />,
    );
  ...
  it("shows a failed state when a step carries an errorCode", () => {
    const { container } = render(
      <ToolTrace steps={[{ toolName: "web_search", query: "X", errorCode: "RATE_LIMIT", columnSlug: "test" }]} />,
    );
