...
describe("UnscheduledTray display", () => {
	...
	it("excludes Done cards with no due date", () => {
		const columns = [
			todoColumn([makeCard({ doneAt: "2026-08-01T00:00:00Z" })]),
		];
		render(
			<UnscheduledTray
				columns={columns}
...
describe("Unscheduled tray drag via CalendarView", () => {
	...
	it("schedules card when dragged from tray to date", async () => {
		...
		const columns = [
			todoColumn([makeCard({ id: 1, title: "No date", version: 1 })]),
		];
		render(
			<MemoryRouter>
				<CalendarView
		...
		await waitFor(() =>
			expect(saveCard).toHaveBeenCalledWith(1, {
				dueDate: "2026-08-25",
				version: 1,
			}),
		);
	...
	it("honors version conflict with inline notice", async () => {
		...
		const columns = [
			todoColumn([makeCard({ id: 1, title: "No date", version: 2 })]),
		];
		render(
			<MemoryRouter>
				<CalendarView
