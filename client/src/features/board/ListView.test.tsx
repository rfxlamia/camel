...
describe("ListView", () => {
		...
		const shuffled = [
			{
				...columns[0]!,
				cards: [
					card(2, { position: 2, title: "Second" }),
					card(1, { position: 1, title: "First" }),
				],
			},
		];
	...
	it("shows overdue styling via data-testid for overdue not-done card", () => {
		...
		const cols = [
			{
				...columns[0]!,
				cards: [card(1, { dueDate: "2026-08-01", doneAt: null })],
			},
		];
		renderListView({ columns: cols });
	...
	it("shows brand empty state when zero columns", () => {
		render(
			<ListView columns={[]} onOpenCard={vi.fn()} onColumnChange={vi.fn()} />,
		);
