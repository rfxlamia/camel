...
**Feature-module convention (issue #129)**: New product code belongs in `client/src/features/<name>/` and `server/src/modules/<name>/`; kernel shared code in `client/src/shared/` and server platform dirs — not new files in type-folders or legacy trees (`server/src/agent/`, etc.). Read [`docs/pocket/adr/2026-09-16-feature-module-convention.md`](docs/pocket/adr/2026-09-16-feature-module-convention.md). Canonical feature names and shrinking kernel-in-waiting paths: [`scripts/feature-modules/map.mjs`](scripts/feature-modules/map.mjs). CI guard: [`scripts/check-feature-modules.mjs`](scripts/check-feature-modules.mjs). Rules: new `.ts`/`.tsx` ≤300 lines; **300-on-touch** on non-trivial edits to files already >300 (extract in same PR; tests and `*.generated.ts(x)` / `/generated/` excluded); public API via each module’s `index.ts` only from outside; one-way imports (modules → kernel-in-waiting allowlist OK, not deep imports into leftover type-folder feature files). Unmapped features fail against the working-tree map. Missing `origin/main` merge-base fails loud — CI needs full fetch. Hotfix order after the convention PR: kernel extract → tracker `git mv` → later splits (#115/#121/BoardContext on next touch) into modules/features. Work-items merge adapter stays kernel (not a product feature). Do not big-bang move everything in the convention PR.
